...
public class UserDao {
	...
	public List<User> getUsersList(User obj) throws Exception {
		...
		try {
			...
			String qry = "SELECT distinct (up.user_id),(select sum((DATEDIFF(minute,(ual.[user_login_time] ) ,(ual.[user_logout_time] ) )))/60 "
					+ "From [MasterDB].[dbo].[user_audit_log] ual where ual.user_id = up.user_id) as minutes,";
					qry = qry +"(select DATEDIFF(DAY,min([user_login_time] ) ,max([user_login_time] ) )  From [MasterDB].[dbo].[user_audit_log] ual where user_id is not null ";
			...
			qry = qry +"(select sum((DATEDIFF(minute,([user_login_time] ) ,([user_logout_time]))))/60 From [MasterDB].[dbo].[user_audit_log] ual where user_id is not null ";
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getUser_id())) {
				qry = qry + " and  ual.user_id = ? ";
				arrSize++;
			}
			...
			qry = qry +	"(select count( up.user_id) From [MasterDB].[dbo].[user_profile] up left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id where up.user_id <> ''"
			...
			+ "(select count( up.user_id) From [MasterDB].[dbo].[user_profile] up left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id where up.user_id <> '' "
			+ " and ua.status <> 'Active' ";
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getUser_id())) {
				qry = qry + " and  up.user_id = ? ";
				arrSize++;
			...
			qry = qry + " ) as inActive_users,up.base_sbu,up.base_project as project_code,up.base_role as user_role,up.base_department as department_code,"
					...
					+ "From [MasterDB].[dbo].[user_audit_log] uuu where uuu.user_id =  up.user_id) as last_login,"
			...
			+ "From [MasterDB].[dbo].[user_profile] up "
			+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id  "
			+ "left join [MasterDB].[dbo].[user_audit_log] ual on up.user_id = ual.user_id  "
			
			+ "left join [MasterDB].[dbo].[project] p on up.base_project = p.project_code  "
			+ "left join [MasterDB].[dbo].[sbu] ss on up.base_sbu = ss.sbu_code  "
			+ "left join [MasterDB].[dbo].[department] dd on up.base_department = dd.department_code  "
			
			+ "left join [MasterDB].[dbo].[user_profile] up1 on up.created_by = up1.user_id "
			+ "left join [MasterDB].[dbo].[user_profile] up3 on up.reporting_to = up3.user_id "
			+ "left join [MasterDB].[dbo].[user_profile] up2 on up.modified_by = up2.user_id  where up.user_id <> '' ";
			...
			qry = qry + " order by up.base_role desc";
	...
	public boolean addUser(User obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO [MasterDB].[dbo].[user_profile] "
		    ...
		    if(count > 0) {
		    	...
		    	String UA_qry = "INSERT INTO [MasterDB].[dbo].[user_accounts] (user_id,user_name,status) VALUES (:user_id,:email_id,:status)";
	...
	public boolean updateUser(User obj) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String updateQry = "UPDATE  [MasterDB].[dbo].[user_profile] set user_name=:user_name,email_id=:email_id,contact_number=:contact_number,"
			...
			if(count > 0) {
				...
				flag = true;
				String updateAuditQry = "UPDATE  [MasterDB].[dbo].[user_accounts] set status=:status where user_id = :user_id ";
				paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	public List<User> getDeptList(User obj) throws Exception {
		...
		try {
			String qry = "SELECT user_role From [MasterDB].[dbo].[user_role] "; 
			int arrSize = 0;
	...
	public User validateUser(User user) throws Exception {
		...
		try{  
			...
			String qry = "select up.id,up.user_id,up.user_name,up.base_role,up.contact_number,up.email_id,up.base_department,department_name,up.base_sbu,up.base_project,"
					+ "up.reward_points,p.project_name,s.sbu_name,up.reporting_to,up1.user_name as reporting_user_name from [MasterDB].[dbo].[user_profile] up "
					+ "LEFT join [MasterDB].[dbo].project p on up.base_project = p.project_code  "
					+ "LEFT join [MasterDB].[dbo].sbu s on up.base_sbu = s.sbu_code  "
					+ "LEFT join [MasterDB].[dbo].[department] d on up.base_department = d.department_code  "
					+ "LEFT join [MasterDB].[dbo].user_accounts ua on up.user_id = ua.user_id  "
					+ "LEFT join [MasterDB].[dbo].user_profile up1 on up.reporting_to = up1.user_id  "
	...
	public User getAllPermissions(String base_role) throws SQLException {
		...
		try{  
			con = dataSource.getConnection();
			String qry = "select role,p_add,p_view,p_edit,p_approvals,p_reports,p_dashboards,p_auto_email From [MasterDB].[dbo].[base_role_permissions] "
					+ "where  role <> '' ";
	...
	public int checkUserLoginDetails(User obj) throws Exception {
		...
		try {
			...
			String qry = "select count(user_id) From [MasterDB].[dbo].[user_audit_log] where user_logout_time is null or  user_logout_time = '' "+ subQry;
			int arrSize = 0;
			...
			if(totalRecords > 0) {
				//String updateQry = "UPDATE  [MasterDB].[dbo].[user_audit_log] set user_logout_time=GETDATE()  where user_id= :user_id and user_logout_time is null or  user_logout_time = '' ";
				//BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private boolean setLastLoginTime(User userDetails) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String insertQry = "if exists(SELECT * From [MasterDB].[dbo].[user_accounts] where user_id= :user_id and user_name= :email_id)            "
					+ "BEGIN            "
					+ " UPDATE  [MasterDB].[dbo].[user_accounts] set last_login_date_time=GETDATE()  where user_id= :user_id  "
					+ "End                    "
					+ "else  "
					+ "begin  "
					+ "INSERT INTO [MasterDB].[dbo].[user_accounts] (user_id,user_name,last_login_date_time) values (:user_id,:email_id,GETDATE())  "
					+ "end ";
	...
	public int getTotalRecords(User obj, String searchParameter) throws Exception {
		...
		try {
			int arrSize = 0;
			String qry = "select count(DISTINCT up.user_id) as total_records From [MasterDB].[dbo].[user_profile] up "
			+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id  "
			+ "left join [MasterDB].[dbo].[user_audit_log] ual on up.user_id = ual.user_id  "
	...
	private boolean updateUserAccounts(User userDetails) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String insertQry = "if exists(SELECT * From [MasterDB].[dbo].[user_accounts] where user_id= :user_id )            "
					+ "BEGIN            "
					+ " UPDATE  [MasterDB].[dbo].[user_accounts] set user_name= :email_id,status= :status  where user_id= :user_id  "
					+ "End                    "
					+ "else  "
					+ "begin  "
					+ "INSERT INTO [MasterDB].[dbo].[user_accounts] (user_id,user_name,status) values (:user_id,:email_id,:status)  "
					+ "end ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(userDetails);		 
		    namedParamJdbcTemplate.update(insertQry, paramSource);
			transactionManager.commit(status);
	...
	public boolean deleteProject(User obj) throws Exception {
		boolean flag = false;
		try {
			String sql = "DELETE From [MasterDB].[dbo].[user_profile] WHERE user_id = ?";
	...
	public List<User> getDeptFilterList(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT  count(user_role) as count ,(select count(user_name) From [MasterDB].[dbo].[user_profile]) as totalCount, user_role From [MasterDB].[dbo].[user_profile] where user_role is not null and user_role <> '' group by user_role "; 
			int arrSize = 0;
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getUser_role())) {
				qry = qry + "and user_role = ? ";
				arrSize++;
	...
	public  boolean UserLoginActions(User obj) throws Exception {
		...
		try {
			...
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String insertQry = "INSERT INTO [MasterDB].[dbo].[user_audit_log] (module_id,module_type,message,user_id,user_session_id,device_type,device_type_no,user_login_time)"
	...
	public  boolean UserLogOutActions(User obj) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String insertQry = "UPDATE  [MasterDB].[dbo]. [user_audit_log] set user_logout_time = GETDATE() where "
	...
	public List<User> getMenuList() throws SQLException {
		...
		try{  
			String qry = "select id, module_name, module_url From [MasterDB].[dbo].[form_menu] where project is null or project = 'safety' order by priority asc ";
	...
	public List<User> getUserFilterList(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT up.user_id,up.user_name From [MasterDB].[dbo].[user_profile] up "
					+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id where up.user_id <> '' ";
			int arrSize = 0;
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getUser_id())) {
				qry = qry + " and up.user_id = ? ";
				arrSize++;
	...
	public List<User> getStatusFilterListFromUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT ua.status From [MasterDB].[dbo].[user_profile] up "
					+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id where up.user_id <> '' ";
			int arrSize = 0;
			if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(obj.getUser_id())) {
				qry = qry + " and up.user_id = ? ";
	...
	public List<User> getReportingTosList(User obj) throws SQLException {
		List<User> menuList = null;
		try{  
			String qry = "select user_id,user_name From [MasterDB].[dbo].[user_profile]";
			menuList = jdbcTemplate.query( qry, new BeanPropertyRowMapper<User>(User.class));
	...
	public List<RoleMapping> getDeptsList() throws SQLException {
		...
		try{  
			String qry = "SELECT department_code ,department_name From [MasterDB].[dbo].[department] d "
	...
	public List<User> getRoleFilterListInUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT up.base_role From [MasterDB].[dbo].[user_profile] up "
					+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id  "
	...
	public List<User> getSBUFilterListInUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT s.sbu_code,sbu_name From [MasterDB].[dbo].[user_profile] up "
					+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id  "
					+ "left join [MasterDB].[dbo].[sbu] s on up.base_sbu = s.sbu_code  "
	...
	public List<User> getProjectFilterListInUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT p.project_code,project_name From [MasterDB].[dbo].[user_profile] up "
					+ "left join [MasterDB].[dbo].[user_accounts] ua on up.user_id = ua.user_id  "
					+ "left join [MasterDB].[dbo].[project] p on p.project_code = up.base_project   "
	...
	public List<User> getRewardsHistory(User user) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT[action],sum(rh.[reward_points]) + 100 as [reward_points],up.[user_name],up.user_id From [MasterDB].[dbo].[rewards_history] rh "
					+ "left join [MasterDB].[dbo].user_profile up on rh.user_id = up.user_id where action like '%Incident%' "
	...
	public boolean addUserSelf(User obj) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String updateQry = "UPDATE  [MasterDB].[dbo].[user_profile] set contact_number=:contact_number,"
			...
			if(count > 0) {
				...
				String logQry = "INSERT INTO [MasterDB].[dbo].[user_audit_log] "
	...
	public List<User> getProjectListForUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT p.project_code,project_name From [MasterDB].[dbo].[project] p "
	...
	public List<User> getDeptListForUser(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT d.department_code ,department_name,assigned_to_sbu From [MasterDB].[dbo].[department] d "
	...
	public List<User> getUserActionsForNotification(User obj) throws Exception {
		List<User> objsList = new ArrayList<User>();
		try {
			String qry = "SELECT TOP (10) module_type,message,user_id,create_date From [MasterDB].[dbo].[user_audit_log]  "
