...
public class RoleMappingDao {
	...
	public List<RoleMapping> getDeptsList(RoleMapping obj) throws SQLException {
		...
		try{  
			String qry = "SELECT department_code ,department_name,assigned_to_sbu,project_code From [MasterDB].[dbo].[department] d "
					+ "  left join [MasterDB].[dbo].project p on d.assigned_to_sbu like CONCAT('%',p.sbu_code, '%') "
	...
	public List<RoleMapping> getRoleMappingsList(RoleMapping obj) throws Exception {
		...
		try {
			...
			String qry =" select distinct rp.id,project,safety_type,employee_code,	role_code,rm.incident_type,rp.status,FORMAT(rp.created_date, 'dd-MMM-yy  HH:mm') as created_date"
					...
					+ " left join [MasterDB].[dbo].[project] p on rp.project = p.project_code "
					+ " left join [MasterDB].[dbo].[department] dt on rp.department_code = dt.department_code "
					+ " left join [MasterDB].[dbo].[user_profile] u on  rp.employee_code = u.user_id "
					+ " left join [MasterDB].[dbo].[role_master] rm on  rp.safety_type = rm.incident_code "
	...
	public List<RoleMapping> getDeptFilterList(RoleMapping obj) throws Exception {
		...
		try {
			String qry = "SELECT  p.department_code,	c.department_name  "
					...
					+ " left join [MasterDB].[dbo].[department] c on  p.department_code = c.department_code "
	...
	public List<RoleMapping> getProjectFilterFromRoleMapping(RoleMapping obj) throws Exception {
		...
		try {
			String qry = "SELECT  distinct p.project,	c.project_name  "
					+ " From [MasterDB].[dbo].[role_mapping] p  "
					+ " left join [MasterDB].[dbo].[project] c on  p.project = c.project_code "
	...
	public List<RoleMapping> getRoleMasterFilterList(RoleMapping obj) throws Exception {
		...
		try {
			String qry = "SELECT distinct p.safety_type,	c.incident_code ,c.incident_type "
					+ " From [MasterDB].[dbo].[role_mapping] p  "
					+ " left join [MasterDB].[dbo].[role_master] c on  p.safety_type = c.incident_code "
	...
	public List<RoleMapping> getempFilterList(RoleMapping obj) throws Exception {
		...
		try {
			String qry = "SELECT distinct p.employee_code,	c.user_name  "
					+ " From [MasterDB].[dbo].[role_mapping] p  "
					+ " left join [MasterDB].[dbo].[user_profile] c on  p.employee_code = c.user_id "
	...
	public boolean addRoleMapping(RoleMapping obj) throws Exception {
		...
		try {
			...
			String insertQry = "INSERT INTO [MasterDB].[dbo].[role_mapping] (project,	safety_type,	department_code,	employee_code,	role_code,status,created_date)"
					+ " VALUES "
			...
			if(count > 0) {
				...
				if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(document_code)) {
					...
					if(!StringUtils.isEmpty(obj) && !StringUtils.isEmpty(document_code)) {
						...
						String insert2Qry = "UPDATE  [MasterDB].[dbo].[safety_ims_workflow] set "
						...
						String insert1Qry = "UPDATE  [MasterDB].[dbo].[safety_ims] set status= :status  where document_code in ("+document_code+") ";
						paramSource = new BeanPropertySqlParameterSource(obj);		 
	...
	private String getDocCode(RoleMapping cObj) throws Exception {
		...
		try{
			con = dataSource.getConnection();
			String contract_updateQry = " SELECT STRING_AGG(QUOTENAME(document_code,''''), ',') as  document_code From [MasterDB].[dbo].[safety_ims] where project_code = ? and department_code = ?"
	...
	public boolean updateRoleMapping(RoleMapping obj) throws Exception {
		...
		try {
			NamedParameterJdbcTemplate namedParamJdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
			String updateQry = "UPDATE  [MasterDB].[dbo].[role_mappiUPDATE  [MasterDB].[dbo]. modified_date= getdate(),status= :update_status where id= :id ";
			BeanPropertySqlParameterSource paramSource = new BeanPropertySqlParameterSource(obj);		 
